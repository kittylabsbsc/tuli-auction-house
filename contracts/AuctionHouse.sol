...
import { IMarket, Decimal } from "@tulilabs/core/dist/contracts/interfaces/IMarket.sol";
import { IMedia } from "@tulilabs/core/dist/contracts/interfaces/IMedia.sol";
...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    // The address of the tuli protocol to use via this contract
    address public tuli;
     ...
     * Constructor
     */
    constructor(address _tuli, address _weth) public {
        require(
            IERC165(_tuli).supportsInterface(interfaceId),
    ...
    nonReentrant
    {
        ...
        // For Tuli Protocol, ensure that the bid is valid for the current bidShare configuration
        if(auctions[auctionId].tokenContract == tuli) {
            require(
                IMarket(IMediaExtended(tuli).marketContract()).isValidBid(
    ...
    /**
     * @notice End an auction, finalizing the bid on Tuli if applicable and paying out the respective parties.
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(auctions[auctionId].tokenContract == tuli) {
            // If the auction is running on tuli, settle it on the protocol
            (bool success, uint256 remainingProfit) = _handleTuliAuctionSettlement(auctionId);
    ...
    function _handleTuliAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        address currency = auctions[auctionId].auctionCurrency == address(0) ? wethAddress : auctions[auctionId].auctionCurrency;
        ...
        IERC20(currency).approve(IMediaExtended(tuli).marketContract(), bid.amount);
        IMedia(tuli).setBid(auctions[auctionId].tokenId, bid);
        uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
        try IMedia(tuli).acceptBid(auctions[auctionId].tokenId, bid) {} catch {
            // If the underlying NFT transfer here fails, we should cancel the auction and refund the winner
            IMediaExtended(tuli).removeBid(auctions[auctionId].tokenId);
            return (false, 0);
        }
